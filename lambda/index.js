...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        ...
        const repromptText = 'Eu nasci em seis de novembro de dois mil e quatorze. E você?'
...
const HasBirthdayLaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        let userTimeZone;
        try {
            userTimeZone = await serviceClientFactory.getUpsServiceClient().getSystemTimeZone(deviceId);
        } catch (error) {
            if (error.name !== 'ServiceError') {
                return handlerInput.responseBuilder.speak("Desculpe. Houve um problema na conexão. Tente novamente mais tarde.").getResponse();
            }
        ...
        let nextBirthday = Date.parse(`${PortugueseMonths[month]} ${day}, ${currentYear}`);
        ...
        let speakOutput = `Feliz aniversário de ${currentYear - year} anos!`;
        if (currentDate.getTime() !== nextBirthday) {
            const diffDays = Math.round(Math.abs((currentDate.getTime() - nextBirthday)/oneDay));
            speakOutput = `Bem-vindo de volta. Faltam ${diffDays} dias até seu aniversário.`
        }
...
const HelpIntentHandler = {
    ...
    handle(handlerInput) {
        const speechText = 'Você pode informar sua data de aniversário para que eu lembre dela.';
